...
class KahnsAlgorithm implements GraphAlgorithm, AlgorithmMeta {
  @override
  GraphAlgoType get type => GraphAlgoType.kahns;

  @override
  String get name => "Kahn's Topological Sort";

  @override
  List<GraphStep> generate(GraphModel graph, String start) {
    return KahnsGenerator.generate(graph);
  }
...
class KahnsGenerator {
  static List<GraphStep> generate(GraphModel graph) {
    ...
    while (queue.isNotEmpty) {
      ...
      steps.add(_step(nodes, edges, queue, null, AlgoPhase.exploration));
    }

    // ---- cycle detection (optional visualization) ----
    if (order.length != graph.nodes.length) {
      // mark cycle nodes
      for (final id in nodes.keys) {
        if (nodes[id]!.state != NodeState.visited) {
          nodes[id] = nodes[id]!.copyWith(state: NodeState.error);
        }
      }
