...
class TreeController extends ChangeNotifier implements VisualController {
  final GraphAlgoSteps _algoSteps;
  final Map<TreeAlgoType, TreeAlgorithm> _algorithms;
  ...
  TreeController({
    ...
    required TreeAlgorithm algorithms,
    required TreeModel graph,
  }) : _algoSteps = algoSteps,
       _algorithms = {algorithms.type: algorithms};

  //{for (final a in algorithms) a.type: a} {
  //   _initialStep = GraphStepFactory.initial(graph);
  // }
  ...
  void startAlgorithm(
    ...
    if (!_algoSteps.has(type)) {
      ...
      final steps = algo.generate(graph);
  ...
  void initWithGraph(TreeModel graph) {
    _timer?.cancel();
    _isPlaying = false;
